            ...
            if [ $msCount -gt 0 ] && [ $lsCount -gt 0 ] ; then
               ...
               case $crackTool in
                     ...
                     aircrack-ng) echo "Using $msHexVal-$lsHexVal to create and sort-u filename ../$HOSTNAME"_sortu.txt""
                                  john -w:$HOMEP/$msWordListDir"-"$msHexVal"-"$shortSeparatorFilename"-"$lsWordListDir"-"$lsHexVal"__"$johnRules"_temp1.txt" --rules:$johnRules -stdout | grep -a '^.\{8\}' | sort -u > ../$HOSTNAME"_sortu.txt"
